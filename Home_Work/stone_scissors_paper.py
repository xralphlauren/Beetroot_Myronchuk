...
class game:
    ...
    def result(self):
        if self.machine_choice in self.dct[self.player_choice]:
            ...
            lb_4['text'] = f'PC point: {self.__class__.my_point}'
        elif self.player_choice in self.dct[self.machine_choice]:
            lb_3['text'] = 'You loose'
            self.__class__.pc_point += 1
            lb_5['text'] = f'Your point: {self.__class__.pc_point}'
        else:
            lb_3['text'] = 'Draw'
...
bt_lizard = tk.Button(root, text='rock', font='25', command=g.lizard)
bt_spock = tk.Button(root, text='rock', font='25', command=g.spock)
