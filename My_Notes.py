...
# all(i > 0 for i in lst) - применяет условине сразу к всем элементам списка и возвращает True/False
...
# разница между генераторами и списками - генераторы хранят в себе последний элемент и правило прерывания ( но не все элементы ) и когда генератор присваивается переменной
...
func(*n) - * даёт возможность записать все елементы списка по одному в кортеж, если передать просто n, то будет передан
список внутри кортежа и нужно будет обращаться сначала к первому индексу кортежа а уже потом к списку( for i in args[0])
_____________
def test ():
    x = 10
    def test1():
...
b = a.deepcopy # Если есть вложеные листые или другие масивы, то id у внутренних массивов будут одинаковыми, и при 
...
str.split(maxspilit=2) - режит конкретное число раз
